...
class ModernGRUModel(nn.Module):
    def __init__(self, input_size, units, output_size, num_layers=1):
        super(ModernGRUModel, self).__init__()
        self.gru = nn.GRU(input_size, units, num_layers, batch_first=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GRU Modern Demo with Sparsity & Persistence")
    ...
    if len(sys.argv) == 1:
        print(f"\n{C_BOLD}{C_BLUE}--- GRU Model Configuration ---{C_END}")
    ...
    flops_stats = {
        "config": {"units": num_units, "layers": num_layers, "epochs": num_epochs, "batch_size": batch_size, "device": str(device), 
                   "sparsity": {"train": sparsity_train, "test": sparsity_test, "mode": sparsity_mode}, "model_type": "gru"},
    ...
    base_filename = f"gru_{dataset_name}_spT{int(sparsity_train*100)}_spTe{int(sparsity_test*100)}_{sparsity_mode}_{num_epochs}e_{num_units}u_{num_layers}L_{device.type}"
    subfolder = f"{int(sparsity_train*100)}_{sparsity_mode}"
    output_dir = os.path.join("results", "gru", dataset_name, subfolder)
    ...
    if is_classification:
        ...
        plt.plot(y_test[0].numpy(), label="True"); plt.plot(prediction[0].detach().cpu().numpy(), label="GRU Pred", linestyle="--")
    plt.title(f"GRU: {dataset_name} | Sparsity Test: {sparsity_test*100:.0f}%"); plt.legend(); plt.grid(True, alpha=0.3)
