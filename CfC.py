...
parser = argparse.ArgumentParser(description="CfC Modern Demo with Fix")
...
if len(sys.argv) == 1:
    print(f"\n{C_BOLD}{C_BLUE}--- Configuration du modèle CfC ---{C_END}")
    num_units = get_input("Nombre de neurones (units)", 32)
...
if is_classification:
    ...
    plt.plot(y_test[0].numpy(), label="True"); plt.plot(prediction[0].cpu().numpy(), label="CfC Pred", linestyle="--")
plt.title(f"CfC: {dataset_name} | MSE: {avg_test_loss:.4f}"); plt.legend(); plt.grid(True, alpha=0.3)

output_dir = os.path.join("results", "cfc", dataset_name); os.makedirs(output_dir, exist_ok=True)
plt.savefig(os.path.join(output_dir, f"cfc_{num_epochs}e_{num_units}u_{num_layers}L_{device.type}.png"))
with open(os.path.join(output_dir, f"cfc_{num_epochs}e_{num_units}u_{num_layers}L_{device.type}.json"), "w") as f: json.dump(flops_stats, f, indent=4)
