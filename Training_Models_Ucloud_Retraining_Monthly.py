...
parser = argparse.ArgumentParser(description="Monthly Resilience Benchmark B200")
...
# --- ARCHITECTURES ---
class MonthlyGAT(nn.Module):
    def __init__(self, node_dim=2, edge_dim=1, hidden=32):
        super().__init__()
        ...
        self.fc = nn.Linear(hidden + edge_dim, 1)
    ...
    def predict_edges(self, node_emb, target_ei, target_ea):
        src, dst = target_ei
        combined = torch.cat([node_emb[src], target_ea], dim=-1)
        return self.fc(combined)

class MonthlyRNN(nn.Module):
    def __init__(self, model_type, in_dim, num_countries, emb_dim=8):
        super().__init__()
        self.model_type = model_type
        self.emb = nn.Embedding(num_countries, emb_dim)
        combined_dim = in_dim + (emb_dim * 2)
    ...
    def forward(self, x, rep, part, hx=None):
        e_r = self.emb(rep).unsqueeze(1)
        e_p = self.emb(part).unsqueeze(1)
        x_combined = torch.cat([x, e_r, e_p], dim=-1)
...
def run_monthly_benchmark():
    print(f"[🚀] Adaptive Monthly Benchmark B200")
    ...
    class Logger(object):
        def __init__(self):
            ...
            log_name = f"training_log_{self.start_time}.txt"
    ...
    models = {
        'GAT': MonthlyGAT(node_dim=2, edge_dim=2).to(DEVICE),
        'CfC': MonthlyRNN('CfC', in_dim=len(seq_features), num_countries=num_nodes).to(DEVICE),
        'LTC': MonthlyRNN('LTC', in_dim=len(seq_features), num_countries=num_nodes).to(DEVICE),
        'LSTM': MonthlyRNN('LSTM', in_dim=len(seq_features), num_countries=num_nodes).to(DEVICE),
        'GRU': MonthlyRNN('GRU', in_dim=len(seq_features), num_countries=num_nodes).to(DEVICE)
    }
    ...
    for p in tqdm(TRAIN_PERIODS, desc="Preparation"):
        ...
        if len(merged) > 1000:
            # --- CRITICAL OPTIMIZATION: Create Macro-Graph (Aggregated by Country) ---
            # This avoids processing millions of edges in the GAT convolution
            ...
            train_blocks.append({
                ...
                'macro_ei': m_ei, 'macro_ea': m_ea # The lightweight graph
            })
        ...
        for b in pbar:
            ...
            for n in model_names:
                ...
                for i in range(0, len(x_gpu), BATCH_SIZE):
                    ...
                    if n == 'GAT':
                        # Fresh graph for every batch to avoid "backward a second time" error
                        node_emb = m.compute_node_emb(node_x, m_ei_gpu, m_ea_gpu)
                        p_out = m.predict_edges(node_emb, torch.stack([rep_gpu[i:end], part_gpu[i:end]]), ea_gpu[i:end])
                    else:
                        p_out, _ = m(x_gpu[i:end].unsqueeze(1), rep_gpu[i:end], part_gpu[i:end])
    ...
    for p in TEST_PERIODS:
        ...
        # --- CRITICAL OPTIMIZATION: Macro-Graph for Inference ---
        ...
        for n, m in models.items():
            ...
            with torch.no_grad():
                if n == 'GAT':
                    # Use lightweight macro-graph for inference embeddings
                    node_emb = m.compute_node_emb(node_x_eval, m_ei_eval, m_ea_eval)
                for i in range(0, len(x_all), BATCH_SIZE):
                    end = i + BATCH_SIZE
                    if n == 'GAT': p_out = m.predict_edges(node_emb, torch.stack([rep_all[i:end], part_all[i:end]]), ea_all[i:end])
                    else: p_out, _ = m(x_all[i:end].unsqueeze(1), rep_all[i:end], part_all[i:end])
                    ...
                    for b_rt in eval_memory[-6:]:
                        ...
                        for i_rt in range(0, len(xr_all), BATCH_SIZE):
                            ...
                            if n == 'GAT':
                                ...
                                out_r = models[n].predict_edges(node_emb_r, torch.stack([rr_all[i_rt:end_rt], pr_all[i_rt:end_rt]]), er_all[i_rt:end_rt])
                            else:
                                out_r, _ = models[n](xr_all[i_rt:end_rt].unsqueeze(1), rr_all[i_rt:end_rt], pr_all[i_rt:end_rt])
        ...
        # Calculate Macro-Graph for this month's memory (for future fine-tuning)
        ...
        # Incremental save for live dashboard
        ...
        with open(os.path.join(SCRIPT_DIR, "results/monthly_adaptive_metrics.js"), "w") as f:
            f.write(f"const b200AdaptiveMetricsData = {json.dumps(metrics, indent=2)};")

    print(f"\n✅ Monthly adaptive benchmark completed.")
