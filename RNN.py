...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RNN Modern Demo with Sparsity & Persistence")
    ...
    flops_stats = {
        "config": {"units": num_units, "layers": num_layers, "epochs": num_epochs, "batch_size": batch_size, "device": str(device), 
                   "sparsity": {"train": sparsity_train, "test": sparsity_test, "mode": sparsity_mode}, "model_type": "rnn"},
    ...
    base_filename = f"rnn_{dataset_name}_spT{int(sparsity_train*100)}_spTe{int(sparsity_test*100)}_{sparsity_mode}_{num_epochs}e_{num_units}u_{num_layers}L_{device.type}"
    ...
    if is_classification:
        ...
        plt.plot(y_test[0].numpy(), label="True"); plt.plot(prediction[0].detach().cpu().numpy(), label="RNN Pred", linestyle="--")
    plt.title(f"RNN: {dataset_name} | Sparsity Test: {sparsity_test*100:.0f}%"); plt.legend(); plt.grid(True, alpha=0.3)
