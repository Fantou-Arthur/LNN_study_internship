...
class ModernRNNModel(nn.Module):
    def __init__(self, input_size, units, output_size, num_layers=1):
        super(ModernRNNModel, self).__init__()
        self.rnn = nn.RNN(input_size, units, num_layers, batch_first=True)
...
parser = argparse.ArgumentParser(description="RNN Modern Demo with Fix")
...
if len(sys.argv) == 1:
    print(f"\n{C_BOLD}{C_BLUE}--- Configuration du modèle Simple RNN ---{C_END}")
...
model = ModernRNNModel(input_size, num_units, output_size, num_layers).to(device)
...
flops_stats = {
    "config": {"units": num_units, "layers": num_layers, "rnn_type": "rnn", "epochs": num_epochs, "batch_size": batch_size, "device": str(device)},
...
if use_profiler:
    import shutil
    if os.path.exists('./log/rnn_profile'): shutil.rmtree('./log/rnn_profile')
    handler = torch.profiler.tensorboard_trace_handler('./log/rnn_profile') if save_trace else None
...
if is_classification:
    ...
    plt.step(range(len(t)), t, label="True"); plt.step(range(len(p)), p, label="RNN Pred", linestyle="--")
else:
    plt.plot(y_test[0].numpy(), label="True"); plt.plot(prediction[0].cpu().numpy(), label="RNN Pred", linestyle="--")
plt.title(f"RNN: {dataset_name} | MSE: {avg_test_loss:.4f}"); plt.legend(); plt.grid(True, alpha=0.3)

output_dir = os.path.join("results", "rnn", dataset_name); os.makedirs(output_dir, exist_ok=True)
plt.savefig(os.path.join(output_dir, f"rnn_{num_epochs}e_{num_units}u_{num_layers}L_{device.type}.png"))
with open(os.path.join(output_dir, f"rnn_{num_epochs}e_{num_units}u_{num_layers}L_{device.type}.json"), "w") as f: json.dump(flops_stats, f, indent=4)
