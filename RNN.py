...
parser = argparse.ArgumentParser(description="Simple RNN Modern Demo with Profiler")
...
if len(sys.argv) == 1:
    print("\n--- Configuration du modèle Simple RNN (Mode Interactif) ---")
...
print(f"Device: {device} | Units: {num_units} | Layers: {num_layers} | Type: Simple RNN")
...
with torch.no_grad():
    ...
    if is_classification:
        ...
        plt.step(range(len(t)), t, label="True", where='post'); plt.step(range(len(p)), p, label=f"RNN Pred", linestyle="--", where='post')
    else:
        plt.plot(y_test[0].numpy(), label="True", linewidth=2, color='blue'); plt.plot(prediction[0].cpu().numpy(), label=f"RNN Pred", linestyle="--", linewidth=2, color='orange')
    ...
    full_title = (f"RNN: {num_layers}L | {num_units}u | {num_epochs}e | Dataset: {dataset_name}\n"
    ...
    output_dir = os.path.join("results", "rnn", dataset_name)
    os.makedirs(output_dir, exist_ok=True)
    tl_suffix = f"_tl{target_loss}" if target_loss > 0 else ""
    base_filename = f"rnn_{num_epochs}e_{num_units}u_{num_layers}L_b{batch_size}_{device.type}_rnn{tl_suffix}"
