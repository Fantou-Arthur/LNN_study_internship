...
# Recherche intelligente du fichier parquet
possible_paths = [
    os.path.join(SCRIPT_DIR, "..", "trade_data.parquet"),           # Si dans un sous-dossier de Internship
    os.path.join(SCRIPT_DIR, "..", "Internship", "trade_data.parquet"), # Si Rsquare est à côté de Internship
    os.path.join(SCRIPT_DIR, "trade_data.parquet"),                 # S'il est dans le même dossier
    "/work/Internship/trade_data.parquet",                          # Chemin absolu Ucloud habituel
    r"C:\Users\afant\Downloads\comtradeExports_updatedH5[240924]-wb.csv\comtradeExports_updatedH5[240924]-wb.csv" # Fallback local Windows
]
...
if DATA_PATH is None:
    raise FileNotFoundError("Impossible de trouver trade_data.parquet sur le serveur ou en local.")

print(f"    📍 Dataset trouvé : {DATA_PATH}", flush=True)
...
def run_scientific_benchmark():
    ...
    metrics = {
        ...
        'models': {m: {
            'predictions_usd': [], 'rolling_mae': [], 'growth_rate': [], 'retrain_events': [], 'time_cost_seconds': 0.0, 'retrain_time_seconds': 0.0,
            'tranche_metrics': {
                'all': {'r2': [], 'smape': []},
                'gte_1M': {'r2': [], 'smape': []},
                'gte_10M': {'r2': [], 'smape': []},
    ...
    try:
        ...
        for yr in EVAL_YEARS:
            ...
            def calc_smape(a, f):
                ...
                res[valid] = 100 * (2 * np.abs(f[valid] - a[valid]) / denom[valid])
            ...
            for m_name in model_names:
                ...
                # New metrics per tranche
                y_pred_np = torch.expm1(y_pred).cpu().numpy().flatten()
                y_true_np = torch.expm1(y_tensor).cpu().numpy().flatten()

                tranches = {
                    'all': np.ones_like(y_true_np, dtype=bool),
                    'gte_1M': y_true_np >= 1_000_000,
                    'gte_10M': y_true_np >= 10_000_000,
                    'gte_100M': y_true_np >= 100_000_000
                }

                for t_name, mask in tranches.items():
                    if mask.sum() > 1:
                        y_t = y_true_np[mask]
                        y_p = y_pred_np[mask]
                        r2 = r2_score(y_t, y_p)
                        smape = calc_smape(y_t, y_p)
                    else:
                        r2 = 0.0
                        smape = 0.0
                    metrics['models'][m_name]['tranche_metrics'][t_name]['r2'].append(float(r2) if not np.isnan(r2) and not np.isinf(r2) else 0.0)
                    metrics['models'][m_name]['tranche_metrics'][t_name]['smape'].append(float(smape) if not np.isnan(smape) and not np.isinf(smape) else 0.0)
    ...
    with open(os.path.join(SCRIPT_DIR, "results/b200_retraining_metrics_detailed.json"), "w") as f:
