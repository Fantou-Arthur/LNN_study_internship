...
parser.add_argument('--epochs', '-e', type=int, default=100, help="Nombre d'époques pour l'entraînement")
...
def run_scientific_benchmark():
    ...
    metrics = {
        ...
        'models': {m: {'predictions_usd': [], 'rolling_mae': [], 'growth_rate': [], 'retrain_events': [], 'time_cost_seconds': 0.0} for m in model_names},
    ...
    try:
        ...
        print(f"\n🚀 Démarrage de l'entraînement (100 époques)...", flush=True)
        for epoch in range(EPOCHS):
            ...
            for block in train_data_blocks:
                ...
                for i in range(0, block['len'], BATCH_SIZE):
                    ...
                    # --- Entraînement du GAT (Correction du bug de rétropropagation) ---
                    t0_gat = time.time()
                    models['GAT'].train()
                    opts['GAT'].zero_grad()
                    
                    # 1. Calcul des embeddings pays (Gradients activés, Macro-Graphe)
                    node_emb = models['GAT'].compute_node_emb(x_gat_gpu, macro_ei_gpu, macro_ea_gpu)
                    
                    # 2. Prédiction sur le batch actuel (Micro-Graphe)
                    ...
                    # 3. Rétropropagation (Le modèle apprend !)
                    loss_g.backward()
                    opts['GAT'].step()
    ...
    with open(os.path.join(SCRIPT_DIR, "results/b200_metrics.js"), "w") as f:
        f.write(f"const b200MetricsData = {json.dumps(metrics, indent=2)};")
