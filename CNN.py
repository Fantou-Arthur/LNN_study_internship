...
if __name__ == "__main__":
    ...
    parser.add_argument("--units", type=int, help="Number of filters")
    ...
    if len(sys.argv) == 1:
        print(f"\n{C_BOLD}{C_BLUE}--- CNN Model Configuration ---{C_END}")
        num_units = get_input("Number of filters (units)", 32)
        num_layers = get_input("Number of layers (layers)", 2)
        activation_name = input("Activation (relu/tanh/sigmoid/leaky_relu) [relu]: ").lower().strip() or "relu"
        num_epochs = get_input("Number of epochs (epochs)", 50)
