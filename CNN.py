...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CNN Modern Demo with Sparsity & Persistence")
    parser.add_argument("--units", type=int, help="Nombre de filtres")
    ...
    if len(sys.argv) == 1:
        print(f"\n{C_BOLD}{C_BLUE}--- Configuration du modèle CNN ---{C_END}")
        ...
        num_units, num_layers, activation_name, num_epochs, target_loss, batch_size, dataset_name = args.units or 32, args.layers, args.activation, args.epochs or 50, args.target_loss, args.batch_size or 128, args.dataset
    ...
    print(f"{C_BOLD}Device:{C_END} {C_GREEN}{device}{C_END} | {C_BOLD}Filters:{C_END} {num_units} | {C_BOLD}Dataset:{C_END} {C_YELLOW}{dataset_name}{C_END}")
    ...
    flops_stats = {
        "config": {"units": num_units, "layers": num_layers, "activation": activation_name, "epochs": num_epochs, "batch_size": batch_size, "device": str(device), 
                   "sparsity": {"train": sparsity_train, "test": sparsity_test, "mode": sparsity_mode}, "model_type": "cnn"},
    ...
    base_filename = f"cnn_{dataset_name}_spT{int(sparsity_train*100)}_spTe{int(sparsity_test*100)}_{sparsity_mode}_{num_epochs}e_{num_units}u_{num_layers}L_{device.type}"
    subfolder = f"{int(sparsity_train*100)}_{sparsity_mode}"
    output_dir = os.path.join("results", "cnn", dataset_name, subfolder)
    ...
    model.eval()
    with torch.no_grad():
        prediction = model(x_test_sparsified[0:1].to(device))
    ...
    if is_classification:
        ...
        plt.plot(y_test[0].numpy(), label="True"); plt.plot(prediction[0].detach().cpu().numpy(), label="CNN Pred", linestyle="--")
    plt.title(f"CNN: {dataset_name} | Sparsity Test: {sparsity_test*100:.0f}%"); plt.legend(); plt.grid(True, alpha=0.3)
