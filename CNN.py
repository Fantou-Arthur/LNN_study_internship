...
class ModernCNNModel(nn.Module):
    def __init__(self, input_size, filters, output_size, num_layers=2, activation='relu'):
        super(ModernCNNModel, self).__init__()
        self.layers = nn.ModuleList()
        curr_channels = input_size
        act_fn = {'relu': nn.ReLU(), 'tanh': nn.Tanh(), 'sigmoid': nn.Sigmoid(), 'leaky_relu': nn.LeakyReLU()}[activation]
        for i in range(num_layers):
            # Utilisation de padding=1 pour conserver la longueur temporelle
            self.layers.append(nn.Conv1d(curr_channels, filters, kernel_size=3, padding=1))
            self.layers.append(nn.BatchNorm1d(filters))
            self.layers.append(act_fn)
            curr_channels = filters
        # On remplace l'AdaptiveAvgPool par une couche de sortie qui préserve la séquence
    ...
    def forward(self, x):
        ...
        # x: [batch, filters, seq_len] -> [batch, seq_len, filters]
        x = x.transpose(1, 2)
        return self.fc(x)
...
parser = argparse.ArgumentParser(description="CNN Modern Demo with Profiler")
parser.add_argument("--units", type=int, default=32, help="Nombre de filtres")
parser.add_argument("--layers", type=int, default=2, help="Nombre de couches")
...
if len(sys.argv) == 1:
    print("\n--- Configuration du modèle CNN (Mode Interactif) ---")
    num_units = get_input("Nombre de filtres (units)", 32)
    num_layers = get_input("Nombre de couches (layers)", 2)
    activation_name = input("Activation (relu/tanh/sigmoid/leaky_relu) [relu]: ").lower().strip() or "relu"
    ...
    num_units, num_layers, activation_name, num_epochs, target_loss, batch_size, dataset_name = args.units, args.layers, args.activation, args.epochs, args.target_loss, args.batch_size, args.dataset
...
print(f"Device: {device} | Filters: {num_units} | Layers: {num_layers} | Activation: {activation_name}")
...
model = ModernCNNModel(input_size, num_units, output_size, num_layers, activation_name).to(device)
...
flops_stats = {
    "config": {"units": num_units, "layers": num_layers, "activation": activation_name, "epochs": num_epochs, "batch_size": batch_size, "device": str(device)},
...
if use_profiler:
    import shutil
    if os.path.exists('./log/cnn_profile'): shutil.rmtree('./log/cnn_profile')
    handler = torch.profiler.tensorboard_trace_handler('./log/cnn_profile') if save_trace else None
...
for epoch in range(num_epochs):
    ...
    for bx, by in dataloader:
        ...
        optimizer.zero_grad(); out = model(bx)
        # On s'assure que out et by ont la même structure pour le calcul de la perte
        loss = criterion(out.view(-1, output_size), by.view(-1)) if is_classification else criterion(out, by)
...
with torch.no_grad():
    ...
    if is_classification:
        ...
        plt.step(range(len(t)), t, label="True", where='post'); plt.step(range(len(p)), p, label=f"CNN Pred", linestyle="--", where='post')
    else:
        plt.plot(y_test[0].numpy(), label="True", linewidth=2, color='blue'); plt.plot(prediction[0].cpu().numpy(), label=f"CNN Pred", linestyle="--", linewidth=2, color='orange')
    ...
    full_title = (f"CNN: {num_layers}L | {num_units}u | {num_epochs}e | Dataset: {dataset_name}\n"
    ...
    output_dir = os.path.join("results", "cnn", dataset_name)
    os.makedirs(output_dir, exist_ok=True)
    tl_suffix = f"_tl{target_loss}" if target_loss > 0 else ""
    base_filename = f"cnn_{num_epochs}e_{num_units}u_{num_layers}L_b{batch_size}_{device.type}_{activation_name}{tl_suffix}"
