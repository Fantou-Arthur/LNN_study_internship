...
parser = argparse.ArgumentParser(description="LSTM Modern Demo with Profiler")
...
if len(sys.argv) == 1:
    print("\n--- Configuration du modèle LSTM (Mode Interactif) ---")
...
print(f"Device: {device} | Units: {num_units} | Layers: {num_layers} | Type: LSTM")
...
flops_stats = {
    "config": {"units": num_units, "layers": num_layers, "rnn_type": "lstm", "epochs": num_epochs, "batch_size": batch_size, "device": str(device)},
...
if use_profiler:
    import shutil
    if os.path.exists('./log/lstm_profile'): shutil.rmtree('./log/lstm_profile')
    handler = torch.profiler.tensorboard_trace_handler('./log/lstm_profile') if save_trace else None
...
with torch.no_grad():
    ...
    if is_classification:
        ...
        plt.step(range(len(t)), t, label="True", where='post'); plt.step(range(len(p)), p, label=f"LSTM Pred", linestyle="--", where='post')
    else:
        plt.plot(y_test[0].numpy(), label="True", linewidth=2, color='blue'); plt.plot(prediction[0].cpu().numpy(), label=f"LSTM Pred", linestyle="--", linewidth=2, color='orange')
    ...
    full_title = (f"LSTM: {num_layers}L | {num_units}u | {num_epochs}e | Dataset: {dataset_name}\n"
    ...
    output_dir = os.path.join("results", "lstm", dataset_name)
    os.makedirs(output_dir, exist_ok=True)
    tl_suffix = f"_tl{target_loss}" if target_loss > 0 else ""
    base_filename = f"lstm_{num_epochs}e_{num_units}u_{num_layers}L_b{batch_size}_{device.type}_lstm{tl_suffix}"
