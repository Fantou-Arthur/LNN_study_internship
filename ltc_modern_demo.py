...
class ModernLTCModel(nn.Module):
    def __init__(self, input_size, units, output_size, num_layers=1):
        super(ModernLTCModel, self).__init__()
        ...
        for i in range(num_layers):
            ...
            self.layers.append(LTC(current_input, wiring, batch_first=True))
...
parser = argparse.ArgumentParser(description="LTC Modern Demo with Profiler Support")
...
if len(sys.argv) == 1:
    print("\n--- Configuration du modèle LTC (Mode Interactif) ---")
...
with torch.cuda.device(device) if device.type == 'cuda' else torch.cpu.amp.autocast():
    try: macs, params = get_model_complexity_info(model, (x_train.shape[1], input_size), as_strings=False, print_per_layer_stat=False, verbose=False); flops_per_sample = macs * 2; flops_method = "ptflops"
        ...
        ops_per_step = (input_size * num_units + num_units * num_units) * 6 * 20 * num_layers
...
with torch.no_grad():
    ...
    if is_classification:
        print(f"Test Accuracy: {accuracy:.2f}%" + (f" | F1: {f1:.4f}" if f1 else ""))
    else:
        print(f"Test MSE: {avg_loss:.6f}" + (f" | MAE: {mae:.6f}" if mae else "") + (f" | RMSE: {rmse:.6f}" if rmse else ""))
    ...
    if is_classification:
        ...
        plt.step(range(len(t)), t, label="True", where='post'); plt.step(range(len(p)), p, label="LTC Pred", linestyle="--", where='post')
    else:
        plt.plot(y_test[0].numpy(), label="True", linewidth=2, color='blue'); plt.plot(prediction[0].cpu().numpy(), label="LTC Pred", linestyle="--", linewidth=2, color='orange')
    ...
    full_title = (f"LTC: {num_layers}L | {num_units}u | {num_epochs}e | Dataset: {dataset_name}\n"
    ...
    output_dir = os.path.join("results", "ltc", dataset_name)
    os.makedirs(output_dir, exist_ok=True)
    tl_suffix = f"_tl{target_loss}" if target_loss > 0 else ""
    base_filename = f"ltc_{num_epochs}e_{num_units}u_{num_layers}L_b{batch_size}_{device.type}{tl_suffix}"
